...
object MachineState {
    ...
    val SEND_REQ     = 1.U(width.W)
    val RECV_RSP     = 2.U(width.W)
    val RECV_CMP     = 3.U(width.W)
    val SEND_DAT     = 4.U(width.W)
    val SEND_ACK     = 5.U(width.W)
    val RECV_RECEIPT = 6.U(width.W)
    val RECV_DAT     = 7.U(width.W)
    val RETURN_DAT   = 8.U(width.W)
    val RETURN_ACK   = 9.U(width.W)
}
...
class TransactionMachine(node: Node, tlParams: TilelinkParams, outstanding: Int)(implicit p: Parameters) extends ZJModule {
    ...
    val isBackTypeMM = task.user(0)
    val isPageTypeNC = task.user(1)
    ...
    txreq.bits.MemAttr    := MemAttr(allocate = false.B, cacheable = false.B, device = !isBackTypeMM, ewa = isPageTypeNC || isBackTypeMM).asUInt
