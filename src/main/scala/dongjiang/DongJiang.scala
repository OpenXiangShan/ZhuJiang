...
class DongJiang(lanNode: Node, bbnNode: Option[Node] = None)(implicit p: Parameters) extends DJModule with ImplicitClock with ImplicitReset {
    ...
    ZJPerf.whenEnabled {
        ...
        assert(
            PopCount(Seq(hnTopDownInternal, hnTopDownNoCExit)) ===
                hnTopDownPressure.asUInt,
            "HN TopDown owner is not closed"
        )
        var xsPerfSeq = Seq(
        ...
        ZJPerf.accumulate(xsPerfSeq)
        ZJPerf.max("zj_hn_pos_occupancy", alrUsePos, true.B)
    }
}
