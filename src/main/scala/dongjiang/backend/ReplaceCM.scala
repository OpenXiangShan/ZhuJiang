...
class ReplaceEntry(implicit p: Parameters) extends DJModule {
    ...
    val io = IO(new Bundle {
        ...
        val sfReplWritePerf     = Option.when(ZJPerf.enabled)(Output(new SFReplacementPerf))
        val sfReplSnpUniquePerf = Option.when(ZJPerf.enabled)(Output(new SFReplacementPerf))
    ...
    when(io.alloc.fire) {
        ...
        alloc.perf.zip(io.alloc.bits.perf).foreach { case (sink, source) => sink := source }
    }
    ...
    io.dataTask.bits.perf.foreach(perf => perf := 0.U.asTypeOf(perf))
    ...
    ZJPerf.whenEnabled {
        val replacementTrace = reg.perf.get
        def setSFReplacementPerf(perf: SFReplacementPerf, event: Bool): Unit = {
            perf.event             := event
            perf.fromSrc           := event && replacementTrace.fromSrc
            perf.fromSnp           := event && replacementTrace.fromSnp
            perf.allocate          := event && replacementTrace.allocate
            perf.noAllocate        := event && !replacementTrace.allocate
            perf.srcHit            := event && replacementTrace.sfSrcHit
            perf.othHit            := event && replacementTrace.sfOthHit
            perf.noSrcOrOthHit     := event && !replacementTrace.sfSrcHit && !replacementTrace.sfOthHit
            perf.readNsd           := event && replacementTrace.opcode === ReadNotSharedDirty
            perf.readUnique        := event && replacementTrace.opcode === ReadUnique
            perf.writeBackFull     := event && replacementTrace.opcode === WriteBackFull
            perf.writeEvictOrEvict := event && replacementTrace.opcode === WriteEvictOrEvict
            perf.otherOpcode := event && replacementTrace.opcode =/= ReadNotSharedDirty &&
                replacementTrace.opcode =/= ReadUnique && replacementTrace.opcode =/= WriteBackFull &&
                replacementTrace.opcode =/= WriteEvictOrEvict
            perf.llcI  := event && replacementTrace.llcState === ChiState.I
            perf.llcSC := event && replacementTrace.llcState === ChiState.SC
            perf.llcUC := event && replacementTrace.llcState === ChiState.UC
            perf.llcUD := event && replacementTrace.llcState === ChiState.UD
        }
        setSFReplacementPerf(io.sfReplWritePerf.get, io.writeDir.fire && reg.isReplSF)
        setSFReplacementPerf(io.sfReplSnpUniquePerf.get, io.cmTaskVec(CMID.SNP).fire && reg.isReplSF)
        ZJPerf.accumulate(
            Seq(
...
class ReplaceCM(implicit p: Parameters) extends DJModule {
    ...
    ZJPerf.whenEnabled {
        def anySFReplacement(perfs: Seq[SFReplacementPerf])(select: SFReplacementPerf => Bool): Bool =
            perfs.map(select).reduce(_ | _)

        def sfReplacementEvents(prefix: String, perfs: Seq[SFReplacementPerf]): Seq[(String, Bool)] =
            Seq(
        ...
        val sfReplWritePerfs     = entries.map(_.io.sfReplWritePerf.get)
        val sfReplSnpUniquePerfs = entries.map(_.io.sfReplSnpUniquePerf.get)
