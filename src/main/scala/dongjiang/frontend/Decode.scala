...
import dongjiang._
import dongjiang.backend.{CMTask, CommitTask, ReqPerf, ReqPerfFamily}
...
class Decode(implicit p: Parameters) extends DJModule {
    ...
    ZJPerf.whenEnabled {
        ...
        val family       = ReqPerf.family(taskReg_s3.chi.opcode, taskReg_s3.chi.isRead, taskReg_s3.chi.isWrite)
        val trackedReq   = isCacheableReq && family =/= ReqPerfFamily.NONE

        cmtPerf.valid        := trackedReq
        cmtPerf.family       := family
        cmtPerf.opcode       := taskReg_s3.chi.opcode
        cmtPerf.llcHit       := respDir_s3.llc.hit
        cmtPerf.sawDMT       := false.B
        cmtPerf.sawDCT       := false.B
        cmtPerf.ingressCycle := ingressCycle
        cmtPerf.frontLatency := perfTimer - ingressCycle

        ZJPerf.distribution(
            "zj_hn_frontend_req_latency",
            perfTimer - ingressCycle,
            trackedReq,
