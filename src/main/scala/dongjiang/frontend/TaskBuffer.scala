...
class TaskBuffer(nrEntries: Int, sort: Boolean, timeout: Int = 8)(implicit p: Parameters) extends DJModule {
    ...
    ZJPerf.whenEnabled {
        ...
        val sendEntryCount  = PopCount(entries.map(_.io.state.value === SEND))
        val waitEntryCount  = PopCount(entries.map(_.io.state.value === WAIT))
        val sleepEntryCount = PopCount(entries.map(_.io.state.value === SLEEP))
        ZJPerf.accumulate(
            Seq(
        ...
        ZJPerf.max("zj_taskbuf_valid_entries", validEntryCount, true.B)
